...
class Tkinter_Game():
    def __init__(self):
        ...
        self.order = {1:'', 2:'', 3:'', 4:''}   #十六進制數字的按鍵順序紀錄
        self.counter = 1                        #十六進制數字按鍵的紀錄數字
    ...
    def front_play(self, root):
        self.poker_allocation(root)
        self.state_allocation(root)
        self.board_allocation(root)
    ...
    def poker_button_active(self):
        ...
        for ctr in range(1,self.counter):
            if self.order[ctr] == '1':
                self.board_button_poker1st.config(state='disabled')
            elif self.order[ctr] == '2':
                self.board_button_poker2nd.config(state='disabled')
            elif self.order[ctr] == '3':
                self.board_button_poker3rd.config(state='disabled')
            elif self.order[ctr] == '4':
                self.board_button_poker4th.config(state='disabled')
            else: #self.order[tmp] == ''
                continue
    ...
    def delete(self):
        try:
            if self.state_string[-1].isalnum():
                self.button_poker_delete()
                self.poker_button_active()
                self.operator_button_disable()
                self.state_string = self.state_string[0:-1]
                #print 'button_poker_delete() called'
            else:
                self.state_string = self.state_string[0:-1]
                self.poker_button_disable()
                self.operator_button_active()
                #print 'button_poker_delete() not called'
            #print self.state
            self.transfer_state()
            #print self.state
    ...
    #刪除操作時按鍵的狀態恢復
    def button_poker_delete(self):
        self.counter -= 1

        if self.order[self.counter] == '1':
            self.board_button_poker1st.config(state='normal')
        elif self.order[self.counter] == '2':
            self.board_button_poker2nd.config(state='normal')
        elif self.order[self.counter] == '3':
            self.board_button_poker3rd.config(state='normal')
        else: #self.order[self.counter] == '4':
            self.board_button_poker4th.config(state='normal')
